...
| File | Source | Notes |
| --- | --- | --- |
| `finchnode-developer-api.openapi.yaml` | https://finchnode.com/openapi.yaml | Authenticated API (`https://api.finchnode.com/api/v1`). Bearer `ck_test_…` / `ck_live_…` keys are **server-side only**; Lunara's browser never holds one. |
