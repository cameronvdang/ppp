...
export interface PregnancyTestPlan {
  /**
   * @deprecated PPP does not calculate an app-authored "early" test date.
...
export function fertilityDayGuide(
  ...
  if (!prediction.ovulationDate || !prediction.fertileWindow) {
    return {
      ...
      relativeDay: null,
      rationale: 'PPP needs enough cycle history to estimate an ovulation window.',
    }
  }
...
/**
 * Plans dates without interpreting a test result. PPP deliberately does
...
export function summarizeBbt(
  ...
  if (readings.length < 7) {
    return {
      ...
      explanation:
        'A few more readings are needed before PPP can look for a sustained rise against your own baseline.',
    }
  }
  return {
    readingsLast30Days: readings.length,
