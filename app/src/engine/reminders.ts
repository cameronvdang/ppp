...
export function copyFor(kind: ReminderKind, mode: ReminderPreview['mode'] = 'private'): {
  title: string
  body: string
} {
  if (mode === 'private') {
  ...
  switch (kind) {
    ...
    case 'prenatal-vitamin':
      return { title: 'PPP reminder', body: 'It is time for a routine you scheduled.' }
    case 'water':
      return { title: 'PPP reminder', body: 'A hydration check-in is ready.' }
    case 'sleep':
      return { title: 'PPP reminder', body: 'Your wind-down check-in is ready.' }
    case 'weight':
    case 'movement':
      return { title: 'PPP reminder', body: 'Your wellness check-in is ready.' }
    case 'journaling':
      return { title: 'PPP reminder', body: 'A quiet reflection is ready when you are.' }
    case 'pregnancy-week':
      return { title: 'PPP update', body: 'Your weekly update is ready.' }
    case 'cycle-estimate':
    case 'period-log':
    case 'bbt':
    case 'opk':
    case 'pregnancy-test':
      ...
      return { title: 'PPP check-in', body: 'A check-in you scheduled is ready.' }
  }
}
