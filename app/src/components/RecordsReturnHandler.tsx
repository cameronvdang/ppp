...
import { OfflineError, requireOnline } from '../platform/offline'
...
export function subscribeRecordsReturn(params: ReturnParams, settled: (error: string | null) => void, deps?: ConnectDeps): () => void {
  ...
  void handleRecordsReturn(params, deps).then(() => { if (active) settled(null) }, error => {
    if (active) settled(error instanceof InvalidReturnError || error instanceof OfflineError ? error.message : 'Could not finish your connection. Check again from Records.')
  })
  return () => { active = false }
}
