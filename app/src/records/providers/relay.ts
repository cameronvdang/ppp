...
export function canonicalizeRelayUrl(value: string): string {
  ...
  if (!(u.protocol === 'https:' || (u.protocol === 'http:' && ['localhost', '127.0.0.1'].includes(u.hostname)))) throw new Error('Use an https relay URL, or http on localhost.')
  if (u.username || u.password || u.search || u.hash || value.includes('?') || value.includes('#')) throw new Error('Relay URLs cannot contain credentials, a query or a fragment.')
  return u.href.replace(/\/+$/, '')
}
export function validateHostedConnectUrl(value: string): string {
  const u = new URL(value)
  if (u.protocol !== 'https:' || u.username || u.password) throw new Error('The records service returned an unsafe connection URL.')
  return u.href
}
...
export function createRelayProvider(config: { baseUrl: string; token: string }, deps: HttpDeps = {}): RecordsProvider {
  ...
  return {
    ...
    async getSession(id) {
      if (!SESSION_ID.test(id)) throw new Error('Invalid connection ID.')
      ...
      if (r.id !== id || !SESSION_STATUSES.includes(r.status as SessionStatus) || !SYNC_STATUSES.includes(sync.status as SyncStatus)) throw new Error('The records service returned an invalid connection state.')
    ...
    async fetchSnapshot(subject, input) {
      ...
      if (!SUBJECT_ID.test(subject)) throw new Error('Invalid records subject.')
      const r = object(await json(`/v1/users/${subject}/records?categories=${categories.join(',')}`))
      if (r.id !== subject) throw new Error('The records service returned a different subject.')
