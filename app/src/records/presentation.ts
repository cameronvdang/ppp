...
export function recordDetail(r: MedicalRecord): string {
  ...
  switch (r.category) {
    case 'demographics': return join(r.birthDate, r.gender, r.address, r.phone, r.email)
    case 'medications': return join(r.dosage, r.frequency, r.status, r.date)
    case 'conditions': return join(r.status, r.onsetDate ? `onset ${r.onsetDate}` : r.recordedDate)
    case 'labs': case 'vitals': return join([r.value === null ? null : String(r.value), r.unit].filter(Boolean).join(' '), r.date)
    case 'allergies': return join(r.reaction, r.severity, r.recordedDate)
    case 'immunizations': return join(r.status, r.date)
  }
}
